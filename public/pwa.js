...
self.addEventListener("install", event => {
    console && console.log("%cmetashow", "color:purple", `Installing app version ${staticCacheName}...`);
});
            ...
            event.respondWith(
                (async () => {
                    try {
                        ...
                        console && console.log("%cmetashow", "color:purple", "Offline", error);
                    }
                })()
            );
        } else {
            ...
            event.respondWith(
                caches.match(event.request).then(response => {
                    ...
                    var cacheBuster =
                        ...
                        ("meta-show.co" === pathUrl.hostname && pathUrl.pathname.match(/(?:^\/(?:css|js|img)\/|\.(?:ico|png|gif|jpe?g|txt|json|woff2)$)/gi));
                    ...
                    return fetch(event.request, { cache: cacheBuster ? "reload" : "no-cache" }).then(response => {
                        ...
                        return caches.open(staticCacheName).then(cache => {
                            console && console.log("%cmetashow", "color:purple", "DLC " + event.request.url);
...
self.addEventListener("activate", event => {
    console && console.log("%cmetashow", "color:purple", `Activating app version ${staticCacheName}...`);
